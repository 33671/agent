...
from queue_utils import Message, MessageType, print_message
...
async def execute_tool_calls(tool_calls, print_queue):
    """执行工具调用"""
    results = []
    ...
    for call in tool_calls:
        ...
        await print_queue.put(print_message(
            f"[Executing tool]: {tool_name}({json.dumps(tool_args)})"
        ))
        ...
        await print_queue.put(print_message(
            f"[Tool call id: {call.id} {tool_name} result]: {result_str}"
        ))
...
async def process_user_message(user_content: str, messages: List[Dict],
                               is_preserved_thinking: bool, print_queue):
    """处理一条用户消息（用户输入或终端输出）"""
    ...
    while True:
        ...
        if asyncio.current_task().cancelled():
            break
        if msg.tool_calls:
            tool_results = await execute_tool_calls(msg.tool_calls, print_queue)
...
async def model_consumer(main_queue: asyncio.Queue, print_queue: asyncio.Queue,user_interrupt_queue: asyncio.Queue,
                         is_preserved_thinking: bool, system_prompt: str):
    ...
    while running:
        ...
        if msg.type == MessageType.COMMAND:
            ...
            # await print_queue.put(print_message(f"\n[收到用户输入]: {msg.data}"))
            process_user_message_task = asyncio.create_task(process_user_message(msg.data, messages, is_preserved_thinking, print_queue))
...
async def print_consumer(print_queue: asyncio.Queue):
    ...
    while True:
        ...
        if msg.type == MessageType.PRINT:
            ...
            # 使用 HTML 支持简单样式，也可以直接用 text
            print_formatted_text(HTML(text), **kwargs)
        else:
